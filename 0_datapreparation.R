...
figure$Sex_title<-ifelse(figure$Sex==0,"Female","Male")
...
####################################
###SUPPLEMENTARY ACTRIVITY TABLES###
####################################

##Now we're going to save the values for Table S3
...
TableS5a<-figure%>%
  ...
  dplyr::select(-play_sum, -food_production_sum, -childcare_sum, -household_sum, -offset_sum)
write.csv(TableS5a,"TableS5a.csv")
...
write.csv(TableS5b,"TableS5b.csv")
...
M3<-d%>%dplyr::select(Middle,Ado,Sex,div,nonforaged)
c1<-as.data.frame(cor(M3))
v1<-as.data.frame(vif(M3))
c1<-cbind(c1,v1)
write.csv(c1,"c1.csv")

M4<-d%>%dplyr::select(dens,water_rating,Middle,Ado,Sex,nonforaged)
c2<-as.data.frame(cor(M4))
v2<-as.data.frame(vif(M4))
c2<-cbind(c2,v2)
write.csv(c2,"c2.csv")

M5<-d%>%dplyr::select(Middle,Ado,Sex,NPP,meanAnnualTemp,totalAnnualPrec,nonforaged)
